function  [T_pf, Q_conv, dp, sections] = spiral_duct(T_pi, T_w, P_i, A, P, L, Dc, m, fluid)
    % same approach as linear duct just changing the formulas
    [T_pf1, Q_conv1, dp1] = singlephase_spiral_duct(T_pi, T_w, P_i, A, P, L,Dc, m, fluid);
    %
    if (T_pf1>fluid.boiling(P_i-dp1) && T_pi < fluid.boiling(P_i))
        ...
        while(abs(T_pf1-fluid.boiling(P_i-dp1))>0.01)
            x_search = (x(1)+x(2))/2;
            [T_pf1, Q_conv1, dp1] = singlephase_spiral_duct(T_pi, T_w, P_i, A, P, x_search,Dc, m, fluid);
        ...
        % use the linear duct two phase function 
        ...
        if(Q_conv2>m*fluid.hvap(P_i-dp1,dp2))
            ...
            [T_pf3, Q_conv3, dp3] = singlephase_spiral_duct(T_pf2, T_w, P_i-dp1-dp2, A, P, L-x_search-x_search2,Dc, m, fluid);
            T_pf = T_pf3;
