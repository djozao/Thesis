...
if alpha_sep > alpha
    ...
    for i = 1:n_3
        ...
        elseif j == 0
            fraction_power_ring(n_2+i,1) = (atan((h_max)/(o+L-(h_max)/tan(gamma)))^2-atan((sin(gamma)*L_D/n_3*(i-1))/(o+L-(sin(gamma)*L_D/n_3*(i-1))/tan(gamma)))^2)/(alpha^2);
            j = 1;
        else
    ...
    for i = 1:n_2
        if L_cyl/n_2*i < L_cyl-x_max
            fraction_power_ring(i,1) = (atan((D/2)/(o+L_cyl-L_cyl/n_2*i))^2-atan((D/2)/(o+L_cyl-L_cyl/n_2*(i-1)))^2)/(alpha^2);
        elseif j == 0
            fraction_power_ring(i,1) = ((alpha)^2-atan((D/2)/(o+L_cyl-L_cyl/n_2*(i-1)))^2)/(alpha^2);
            j = 1;
        else
