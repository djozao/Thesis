function residuals = Teq_cone_radcond(T, n_1, n_2, emi, L_D, D, A, k, t, B_matrix, Heat, T_0)
    ...
    for j = 1:n_1
        if n_1 == 1
            ...
            + (T(1+j+n_2)-T(1+j))* 2*pi*k*t*tan(gamma)/(log( (2*D/2+t)   /  (2*(D/2-sin(gamma)*L_D/2/n_2)+t)));
    ...
    for j = 1:n_2
        if n_2 == 1
            residuals(n_1 + j) = Heat(n_1 + j) - emi * (pi*D/2*L_D*(j^2-(j-1)^2)/(n_2^2)) * stef * (B_matrix(1 + n_1 + j, :) * (T(1 + n_1 + j)^4 - T.^4))...
                                ...
                                + (T(1+n_1)-T(1+j+n_1))* 2*pi*k*t*tan(gamma)/(log((2*D/2+t)   /  (2*(D/2-sin(gamma)*L_D/2/n_2)+t)));
        ...
        elseif j == n_2
            ...
            residuals(n_1 + j) = Heat(n_1 + j) - emi * (pi*D/2*L_D*(j^2-(j-1)^2)/(n_2^2)) * stef * (B_matrix(1 + n_1 + j, :) * (T(1 + n_1 + j)^4 - T.^4))...
                ...
                + (T(1+n_1+j+1)-T(1+j+n_1))* 2*pi*k*t*tan(gamma)/(log( (sin(gamma)*L_D/2/n_2 + sin(gamma)*L_D/n_2*(j+1)) / (sin(gamma)*L_D/2/n_2 + sin(gamma)*L_D/n_2*(j) )  ));
