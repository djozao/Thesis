...
intervals = [88, 98; 165, 175; 233, 243; 352, 362; 410, 420]; % Define your intervals here
...
for i = 1:size(intervals, 1)
    fprintf('Average mass flow between %d and %d: %.3f mg/s with %.3f deviation \n', intervals(i, 1), intervals(i, 2), averaged_mass_flow(i), std_mass_flow(i));
end

%% Pressure

filename = '20240325 SECOND\pressure_2.txt'; % Replace 'your_file.txt' with the actual filename
...
% Add labels and title
xlabel('Time (s)');
ylabel('Inlet Absolute Pressure (bar)');
...
intervals = [70, 96; 146, 172; 214, 240; 332, 359 ; 391, 417]; % Adjust as needed
...
% Display results
for i = 1:size(intervals, 1)
    fprintf('Interval %d: Average Pressure = %.3f, Standard Deviation = %.3f\n', i, avg_pressure(i), std_pressure(i));
end
%%
...
time_intervals_no_load = [0, 67; 107, 144; 183, 211; 253, 328; 371, 388];
time_intervals_load = [73, 98; 148, 174 ; 216, 242 ; 335, 361 ; 394, 419];
...
%% Calculation of experimental generated thrust
...
%% Expected Thrust and other values
gamma_nitrogen = 1.40;
...
viscosity = 1.7367*10^-5;
density = 3.5108; % 4.6823, 5.8544, 7.0272
% 1.7381e-05; 1.7395e-05; 1.7410e-05;
...
f_third_alternative = 0.3164*Re_third^-0.25
