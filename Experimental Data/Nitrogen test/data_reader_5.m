...
grid on;
% windowSize_mf = 100;
% smoothed_mf= movmean(massflow,windowSize_mf);
% % Plot original and smoothed data
% plot(time_mf, massflow, 'b', 'LineWidth', 1.5);
% hold on;
% %plot(time_mf, smoothed_mf, 'r', 'LineWidth', 2);
% xlabel('X');
% ylabel('Y');
...
intervals = [46, 56; 107, 117; 167, 177; 229, 239 ; 284, 294]; % Define your intervals here
...
filename = '20240325 FIFTH\pressure_5.txt'; % Replace 'your_file.txt' with the actual filename
...
xlabel('Time (s)');
ylabel('Absolute Inlet Pressure (bar)');
...
intervals = [24, 54; 86, 115; 144, 174; 204, 234; 264, 291]; % Adjust as needed
...
time_intervals_no_load = [0, 23; 67, 84; 127, 144; 188, 203; 249, 263];
time_intervals_load = [27, 55; 90, 115 ; 148, 175 ; 208, 237 ; 269, 293];
...
viscosity_throat = 1.5040*10^-5;

Re_throat = 4*mean(mf_adjusted)/viscosity_throat/(pi*nozzle_diameter)


displacement_exp = 1.72*3*10^-3/sqrt(Re_throat);

1-2*displacement_exp/nozzle_diameter/2

%%

mf_mean = mean(mf_adjusted)
viscosity = 1.7410*10^-5;
density =  7.0272;
